    ...
    if (index[key] != 0)
    {
        ...
        while (true)
        {
            ...
            if (select == 1)
                ...
                system(std::string("start " + Hash->table[index[key]].data).c_str());
            else if (select == 3)
                menu();
            else
                exit(0);
...
void createMatrix()
{
    ...
    for (int i = 1; i < n; i++)
    {
        for (int j = 0; j < Hash->table[i].count; j++)
        {
            matrix[i][Hash->table[i].idlk[j]] = 1;
            matrix[Hash->table[i].idlk[j]][i] = 1;
        }
    }
}
void find_way(int* way, int count)
...
void check2url()
{
    ...
    SetTeColor(4);
    cout << "  " << Hash->table[id1].data;
    SetTeColor(15);
    id2 = selection(1, n - 1, 2);
    SetTeColor(4);
    cout << "  " << Hash->table[id2].data;
    SetTeColor(15);
...
void deleteURL(int id) {
    ...
    for (int i = 1; i < Hash->table[move_id[m - 1]].count; i++)
        if (Hash->table[move_id[m - 1]].idlk[i] == index[key])
        {
            idele = i;
            break;
        }
    ...
    if (move_id[m - 1] != 0)
    {
        for (int i = idele; i < Hash->table[move_id[m - 1]].count - 1; i++)
        {
            Hash->table[move_id[m - 1]].idlk[i] = Hash->table[move_id[m - 1]].idlk[i + 1];
        }
        Hash->table[move_id[m - 1]].count--;
    }
    move_id[m] = 0;
    m--;
    move_url(move_id[m]);
...
void move_url(int id)
{
    ...
    for (int i = 1; i < m; i++)
    {
        cout << "------" << Hash->table[move_id[i]].data << endl
            << "VVVVVVVVV" << endl;
        y += 2;
    }
    if (index[string_tokey(Hash->table[id].data)] != 0)
    {
        cout << "------" << Hash->table[id].data << "\n\n";
        y += 2;
    }
    cout << "Linked URLs: " << endl << endl;
    y += 2;
    int i = 1;
    if (Hash->table[id].count == 1)
    {
        cout << "\n   Not found URL\n\n";
        y += 3;
    }
    ...
    {
        for (i = 1; i < Hash->table[id].count; i++)
        {
            int idshow = Hash->table[id].idlk[i];
            cout << "   " << i << "   " << Hash->table[idshow].data << endl;
        }
    }
    cout << "   Access to URL" << endl
        << "    Insert URL" << endl
    ...
    while (true)
    {
        select = selection(1, Hash->table[id].count + 6, y);
        if (select >= 1 && select < i)
        {
            move_url(Hash->table[id].idlk[select]);
        }
        else if (select == i)
        {
            system(std::string("start " + Hash->table[id].data).c_str());
            system("pause>>null");
        }

        else if (select == i + 1)
        {
            gotoxy(0, Hash->table[id].count + y + 5);
            cout << "Enter URL:";
            string S;
            cin >> S;
            insertChild(Hash->table[id].data, S);
            move_url(id);
        }
        else if (select == i + 2)
        {
